package com.gxq.learn.logger.utils;
...
public class MyMultipleTextOutputFormat<K, V> extends MultipleTextOutputFormat<K, V> {
	...
	public RecordWriter getRecordWriter(final FileSystem fs, final JobConf job, final String name,
			final Progressable arg3) throws IOException {
	...
	protected RecordWriter<K, V> getBaseRecordWriter(FileSystem fs, JobConf job, String path, Progressable arg3)
			throws IOException {
	...
	public void checkOutputSpecs(FileSystem ignored, JobConf job) throws IOException {
		...
		if (outDir != null) {
			...
			// check its existence
			/*
			 * if (fs.exists(outDir)) { throw new
			 * FileAlreadyExistsException("Output directory " + outDir + " already exists");
			 * }
			 */
		}
	}
